...
describe("typesDao", () => {
  ...
  describe("addType", () => {
    ...
    it("should add a new document type successfully", async () => {
      db.run.mockImplementation((sql, params, callback) => {
        callback.call({ lastID: mockInsertedId }, null); // Mock `this.lastID`
      });

      const result = await typesDao.addType(type, iconSrc);
      expect(result).toEqual({
        IdType: mockInsertedId,
        Type: type,
        IconSrc: iconSrc,
      });
    });
    ...
    it("should reject if there is a database error", async () => {
      ...
      await expect(typesDao.addType(type, iconSrc)).rejects.toThrow("DB error");
    });
  });
});
